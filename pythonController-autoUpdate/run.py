...
def __main__():
    ...
    bottom_button = tk.Button(bottom_frame, text='離開', fg='black', command=quit)
    # 讓系統自動擺放元件（靠下方）
...
class setting:
    ...
    def __init__(self,mode):
        if mode is str:
            self.m = mode
            window.title("setting:socket-game-%s_2P"%(mode))
