class cheakFile:
    ...
    def find_dir(self,from_file_path,correspond_file_path):
        ...
        if not self.from_dir_list == []:
            for from_dir_list in self.from_dir_list:
                time.sleep(0.5)
                t1 = threading.Thread(target = self.find_dir(from_dir_list,correspond_file_path))
                rlock1 = t1.RLock()
    ...
    def setup(self,from_file_path,correspond_file_path):
        import os
        if not os.path.isfile(from_file_path) or not os.path.isfile(correspond_file_path):
            return None
