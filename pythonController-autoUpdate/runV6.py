...
def init():
    ...
    ipList = random.sample(list(net4),len(list(net4))-1)
    for ip in ipList:
        ip_format = list(net4)[ip]#+":"+rdn
        if not os.path.exists("ip"):
            with open("ip", "w") as ipf:
                ipf.write(ip_format)
        if ip_format in net4.hosts():
            print(ip_format)
            #ip = ipaddress.ip_address(ip_format)
            if ip_format.is_link_local:
                print(ip_format.broadcast_address)
            ipaddress.ip_network(ip_format, strict=True)
            print(socket.gethostbyaddr(str(ip_format)))
            break
init()
window = tk.Tk()
...
class client(setup,tk.Frame):
    ...
    def setup_game_client(self):
        ...
        if ans == "":
            ans = abs(int(ans))
            if int(ans) < low or int(ans) >= high or ans == "":
                return True
            return False
